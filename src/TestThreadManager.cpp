...
int main(int argc, const char * argv[]) {
    ...
    ProcessControl control(2);

    std::shared_ptr<IData> data = std::make_shared<Data>();

    std::shared_ptr<IProcess> process1 = std::make_shared<ProcessTest>("A",  500, "test 1");
    std::shared_ptr<IProcess> process2 = std::make_shared<ProcessTest>("B",  10,  "test 2");
    std::shared_ptr<IProcess> process3 = std::make_shared<ProcessTest>("C",  40,   "test 3");
    std::shared_ptr<IProcess> process4 = std::make_shared<ProcessTest>("D1", 500, "test D1");
    std::shared_ptr<IProcess> process5 = std::make_shared<ProcessTest>("D2", 500, "test D2");
    std::shared_ptr<IProcess> process6 = std::make_shared<ProcessTest>("D3", 500, "test D3");
    std::shared_ptr<IProcess> process7 = std::make_shared<ProcessTest>("D4", 500, "test D4");


//Data

    control.AddProcess("E",
                       [](std::shared_ptr<IData> pIData){
